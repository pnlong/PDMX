...
if __name__ == "__main__":
    ...
    dataset["rating"] = list(map(dataset_full_analysis.discretize_rating, dataset["rating"]))
    ...
    df = dataset_full_analysis.group_by(df = dataset, by = "rating")
    ...
    # create plot
    mosaic = list(zip(dataset_full.MMT_STATISTIC_COLUMNS)) if args.column else [dataset_full.MMT_STATISTIC_COLUMNS]
    ...
    # make plots
    for mmt_statistic_column in dataset_full.MMT_STATISTIC_COLUMNS[::-1]:
        ...
        elif args.column and (mmt_statistic_column != dataset_full.MMT_STATISTIC_COLUMNS[-1]):
            axes[mmt_statistic_column].sharex(other = axes[dataset_full.MMT_STATISTIC_COLUMNS[-1]])
        ...
        axes[mmt_statistic_column].set_xlim(left = 3.0 - (0.5 * dataset_full_analysis.RATING_ROUND_TO_THE_NEAREST), right = 5.0 + (0.5 * dataset_full_analysis.RATING_ROUND_TO_THE_NEAREST))
    ...
    # save image
    plots_dir = f"{dirname(args.dataset_filepath)}/{dataset_full_analysis.PLOTS_DIR_NAME}"

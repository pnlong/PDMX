...
def scrape_annotations(annotations: List[Annotation], song_length: int, use_implied_duration: bool = True) -> pd.DataFrame:
    ...
    annotations_encoded = {key: [] for key in representation.DIMENSIONS} # create dictionary of lists
    ...
    return pd.DataFrame(data = annotations_encoded, columns = representation.DIMENSIONS)
...
def scrape_barlines(barlines: List[Barline], song_length: int, use_implied_duration: bool = True) -> pd.DataFrame:
    ...
    barlines_encoded = {key: utils.rep(x = None, times = len(barlines)) for key in representation.DIMENSIONS} # create dictionary of lists
...
def scrape_tempos(tempos: List[Tempo], song_length: int, use_implied_duration: bool = True) -> pd.DataFrame:
    ...
    tempos_encoded = {key: utils.rep(x = None, times = len(tempos)) for key in representation.DIMENSIONS} # create dictionary of lists
...
def scrape_notes(notes: List[Note]) -> pd.DataFrame:
    ...
    notes_encoded = {key: utils.rep(x = None, times = len(notes)) for key in representation.DIMENSIONS} # create dictionary of lists
    ...
    return pd.DataFrame(data = notes_encoded, columns = representation.DIMENSIONS) # create dataframe from scraped values
...
def scrape_articulations(annotations: List[Annotation], maximum_gap: int, articulation_count_threshold: int = 4) -> pd.DataFrame:
    ...
    articulations_encoded = {key: [] for key in representation.DIMENSIONS} # create dictionary of lists
    encounters = {}
    ...
    return pd.DataFrame(data = articulations_encoded, columns = representation.DIMENSIONS) # create dataframe from scraped values
...
def scrape_slurs(annotations: List[Annotation], minimum_duration: float, music: BetterMusic) -> pd.DataFrame:
    ...
    slurs_encoded = {key: [] for key in representation.DIMENSIONS} # create dictionary of lists
    for annotation in annotations:
    ...
    return pd.DataFrame(data = slurs_encoded, columns = representation.DIMENSIONS) # create dataframe from scraped values
...
def scrape_pedals(annotations: List[Annotation], minimum_duration: float, music: BetterMusic) -> pd.DataFrame:
    ...
    pedals_encoded = {key: [] for key in representation.DIMENSIONS} # create dictionary of lists
    for annotation in annotations:
    ...
    return pd.DataFrame(data = pedals_encoded, columns = representation.DIMENSIONS) # create dataframe from scraped values
...
def get_system_level_expressive_features(music: BetterMusic, use_implied_duration: bool = True) -> pd.DataFrame:
    ...
    system_time_signatures = scrape_time_signatures(time_signatures = music.time_signatures, song_length = music.song_length, use_implied_duration = use_implied_duration)
    system_key_signatures = scrape_key_signatures(key_signatures = music.key_signatures, song_length = music.song_length, use_implied_duration = use_implied_duration)
...
def extract_data(music: BetterMusic, use_implied_duration: bool = True) -> np.array:
    ...
    output = np.empty(shape = (0, len(representation.DIMENSIONS)), dtype = np.object_)

    # time column index
    time_dim = representation.DIMENSIONS.index("time")
    ...
    for track in music.tracks:
        ...
        data = pd.concat(objs = (pd.DataFrame(columns = representation.DIMENSIONS), system_level_expressive_features, staff_level_expressive_features), axis = 0, ignore_index = True) # combine system and staff expressive features
        ...
        data = data.drop_duplicates(subset = representation.DIMENSIONS[:time_dim], keep = "first", ignore_index = True)
